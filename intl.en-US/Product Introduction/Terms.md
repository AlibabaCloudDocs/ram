...
|identity|RAM provides three types of identities: RAM user, RAM user group, and RAM role. RAM users and user groups are physical identities. RAM roles are virtual identities.|
...
|multi-factor authentication \(MFA\)|A best practice that adds an extra layer of protection in addition to your username and password. MFA enhances security for your account. If MFA is enabled for a user, the user must perform the following operations when the user logs on to the Alibaba Cloud Management Console:

1.  First factor: Enter the username and password of the RAM user.
2.  Second factor: Enter the verification code that is generated by the virtual MFA device, or pass the Universal 2nd Factor \(U2F\) authentication.
...
|RAM role|A virtual identity that you can create within your Alibaba Cloud account. RAM roles, entity users, such as Alibaba Cloud accounts, RAM users, or Alibaba Cloud services, and textbook roles have the following differences:
...
-   **Alibaba Cloud account**: RAM users of a trusted Alibaba Cloud account can assume this type of RAM role. RAM users who assume this type of RAM role can belong to their parent Alibaba Cloud account or another Alibaba Cloud account. This type of RAM role is used for cross-account access and temporary authorization.
